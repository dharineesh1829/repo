-- =========================================================================
-- ResumeMatch AI - Unified Supabase Master Database Setup
...
-- =========================================================================
-- 2. INTERVIEW SESSION QUESTIONS TABLE ("QUTION")
-- =========================================================================
...
CREATE INDEX IF NOT EXISTS idx_tqb_lang_diff_active 
    ON public.technical_question_bank (language, difficulty) 
    WHERE is_active = TRUE;

CREATE INDEX IF NOT EXISTS idx_tqb_lang_topic 
    ON public.technical_question_bank (language, topic)
    WHERE is_active = TRUE;

CREATE INDEX IF NOT EXISTS idx_tqb_tags_gin 
    ON public.technical_question_bank USING GIN (tags);

CREATE INDEX IF NOT EXISTS idx_tqb_content_hash 
    ON public.technical_question_bank (content_hash);

-- =========================================================================
-- 6. FULL READ/WRITE PERMISSIONS FOR PUBLISHABLE KEY & SERVICE ROLE
-- =========================================================================
...
-- 1. Employee Data Policies
DROP POLICY IF EXISTS "Allow all on employee_data" ON public.employee_data;
CREATE POLICY "Allow all on employee_data" ON public.employee_data
...
-- 3. Performance Details Policies
DROP POLICY IF EXISTS "Allow all on performance_details" ON public.performance_details;
CREATE POLICY "Allow all on performance_details" ON public.performance_details
    FOR ALL TO anon, authenticated, service_role USING (true) WITH CHECK (true);

-- 4. Technical Question Bank Policies (Read, Insert, Update, Upsert)
DROP POLICY IF EXISTS "Allow all on technical_question_bank" ON public.technical_question_bank;
DROP POLICY IF EXISTS "Allow public read access to active questions" ON public.technical_question_bank;
DROP POLICY IF EXISTS "Allow service role full access" ON public.technical_question_bank;
DROP POLICY IF EXISTS "Allow anon batch insert" ON public.technical_question_bank;

CREATE POLICY "Allow all on technical_question_bank" ON public.technical_question_bank
    FOR ALL TO anon, authenticated, service_role USING (true) WITH CHECK (true);
